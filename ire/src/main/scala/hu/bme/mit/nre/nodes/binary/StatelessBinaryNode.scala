...
abstract class StatelessBinaryNode(val expectedTerminatorCount: Int = 2) extends Actor with Forwarder with Stash with TerminatorHandler {
    ...
    case Primary(reteMessage: ReteMessage) => {
      primaryPause match {
        ...
        case None => reteMessage match {
          ...
          case cs: BatchChangeSet => onPrimary(cs); // printForwarding(cs)
    ...
    case Secondary(reteMessage: ReteMessage) => {
      secondaryPause match {
        ...
        case None => reteMessage match {
          ...
          case cs: BatchChangeSet => onSecondary(cs); //printForwarding(cs)
          case t: TerminatorMessage => handleTerminator(t)
        }
