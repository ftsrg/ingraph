...
class TerminatorTest(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  "Binary nodes" must {
    "propagate terminator messages" in {
      ...
      val intermediary = system.actorOf(Props(new JoinNode(checker ! _, 1, 1, Vector(0), Vector(0))), "intermediary")
      val input1 = system.actorOf(Props(new JoinNode(intermediary ! Primary(_), 1, 1, Vector(0), Vector(0))), "inputBeta")
