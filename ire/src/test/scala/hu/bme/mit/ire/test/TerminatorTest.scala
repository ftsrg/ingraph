...
class TerminatorTest(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  "Unary nodes" must {
    "propagate terminator messages" in {
      ...
      input1 ! Δ(positive = tupleBag(tuple(15)))
      input1 ! Δ(positive = tupleBag(tuple(19)))
      val input2 = system.actorOf(Props(new σNode(intermediary ! _, c => true)))
      input2 ! Δ(positive = tupleBag(tuple(25)))
      input2 ! Δ(positive = tupleBag(tuple(29)))
      ...
      input1 ! Δ(positive = tupleBag(tuple(16)))
      input1 ! Δ(positive = tupleBag(tuple(17)))
      input2 ! Δ(positive = tupleBag(tuple(26)))
      input2 ! Δ(positive = tupleBag(tuple(27)))
  ...
  "Binary nodes" must {
    "propagate terminator messages" in {
      ...
      val intermediary = system.actorOf(Props(new ⋈(checker ! _, 1, 1, mask(0), mask(0))), "intermediary")
      val input1 = system.actorOf(Props(new ⋈(intermediary ! Primary(_), 1, 1, mask(0), mask(0))), "inputBeta")
      val msg15 = Δ(positive = tupleBag(tuple(15)))
      input1 ! Primary(msg15)
      input1 ! Secondary(msg15)
      intermediary ! Secondary(msg15)
      val msg25 = Δ(positive = tupleBag(tuple(25)))
      ...
      val future = terminator.send()
      input1 ! Primary(Δ(positive = tupleBag(tuple(16))))
      input1 ! Secondary(Δ(positive = tupleBag(tuple(16))))
      intermediary ! Secondary(Δ(positive = tupleBag(tuple(16))))
