...
class DuplicateEliminationNodeTest(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  "DuplicateElimination node" must {
    ...
    "do simple duplicate elimination 1" in {
      ...
      duplicateElimination ! Δ(positive = tupleBag(tuple(1)))
      expectMsg(Δ(positive = tupleBag(tuple(1))))

      duplicateElimination ! Δ(positive = tupleBag(tuple(1)))
      duplicateElimination ! Δ(positive = tupleBag(tuple(1)))

      duplicateElimination ! Δ(negative = tupleBag(tuple(1)))
      duplicateElimination ! Δ(negative = tupleBag(tuple(1)))
      duplicateElimination ! Δ(negative = tupleBag(tuple(1)))
      expectMsg(Δ(negative = tupleBag(tuple(1))))
    }
