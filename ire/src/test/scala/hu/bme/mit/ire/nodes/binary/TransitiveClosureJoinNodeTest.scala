...
class TransitiveClosureJoinNodeTest(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  "A TransitiveClosureNode" must {
    "calculate transitive closure" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(
        positive = tupleBag(tuple(1, 100, 2), tuple(2, 101, 3))
      ))
      expectNoMsg(timeout)

      transitiveClosure ! Primary(IncrementalChangeSet(
    ...
    "discard paths with same edge twice" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(1, 100, 2), tuple(2, 101, 3))))
      expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
        positive = tupleBag(tuple(1, Path(100), 2), tuple(1, Path(100, 101), 3), tuple(2, Path(101), 3))
      )): _*)

      transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(3, 102, 1))))
      expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(2, 103, 4))))
      expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
    ...
    "include paths going through same vertex more times" in {
      ...
      val transitiveClosure = system.actorOf(Props(new TransitiveClosureJoinNode(echoActor ! _, primaryTupleWidth, secondaryTupleWidth, primaryMask, secondaryMask, outputTupleWidth)))
          ...
          transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(1, 100, 2), tuple(2, 101, 1))))
          expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
            positive = tupleBag(tuple(1, Path(100), 2), tuple(1, Path(100, 101), 1), tuple(2, Path(101, 100), 2), tuple(2, Path(101), 1))
          )): _*)

          transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(2, 102, 1))))
          expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
          ...
          transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(2, 103, 3))))
          expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
            positive = tupleBag(tuple(1, Path(100, 103), 3), tuple(2, Path(102, 100, 103), 3), tuple(2, Path(101, 100, 103), 3), tuple(2, Path(103), 3))
          )): _*)

          transitiveClosure ! Secondary(IncrementalChangeSet(negative = tupleBag(tuple(2, 101, 1))))
          expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
    ...
    "not do anything in case of non-present vertices or edges" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(negative = tupleBag(tuple(2, 1010, 3))))
      expectNoMsg(timeout)

      transitiveClosure ! Secondary(IncrementalChangeSet(negative = tupleBag(tuple(0, 100, 2))))
      expectNoMsg(timeout)
    }
    ...
    "include directed cycles" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(negative = tupleBag(tuple(2, 101, 3))))
      expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
    ...
    "handle multiple paths and edges between vertices" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(1, 102, 3), tuple(2, 103, 3))))
      expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
    ...
    "calculate negative updates" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(negative = tupleBag(tuple(1, 100, 2))))
      expectMsg(IncrementalChangeSet(
    ...
    "handle multiple target paths from same target vertex" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(17, 2, 21), tuple(21, 4, 38))))
      expectNoMsg(timeout)

      transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(11, 1, 17))))
      expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
    ...
    "handle multiple source paths from same source vertex" in {
      ...
      transitiveClosure ! Secondary(IncrementalChangeSet(positive = tupleBag(tuple(11, 1, 21))))
      expectMsgAnyOf(Utils.changeSetPermutations(IncrementalChangeSet(
