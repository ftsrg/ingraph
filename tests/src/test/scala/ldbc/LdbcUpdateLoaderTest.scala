...
class LdbcUpdateLoaderTest extends FunSuite {
  ...
  test("parse streams") {
    ...
    val updates: Iterable[LdbcUpdate] = for (update <- loader.getUpdates.asScala) yield {
      ...
      updateType match {
        ...
        case 2 => Like       (u(3), u(4), u(5))
        case 3 => Like(u(3), u(4), u(5))
  ...
  def update(person: Update1AddPerson): Unit = {
    ...
    indexer.addVertex(p)
    indexer.addEdge(indexer.newId(), person.personId, person.cityId, "isLocatedIn")
    for (tag <- person.tagIds)
      indexer.addEdge(indexer.newId(), person.personId, tag, "hasInterest")
  ...
  def update(forum: Update4AddForum): Unit = {
    ...
    indexer.addEdge(indexer.newId(), forum.forumId, forum.moderatorPersonId, "hasModerator")
    for (tag <- forum.tagIds)
      indexer.addEdge(indexer.newId(), forum.forumId, tag, "hasTag")
  }

  def update(member: Update5AddForumMembership): Unit = {
    indexer.addEdge(indexer.newId(), member.personId, member.forumId, "hasMember", Map("joinDate" -> member.joinDate))
  }
  ...
  def update(post: Update6AddPost): Unit = {
    indexer.addVertex(IngraphVertex(post.postId, Set("Post"),
      ...
      "length" -> post.length)))
    indexer.addEdge(indexer.newId(), post.postId, post.authorPersonId, "hasCreator")
    indexer.addEdge(indexer.newId(), post.forumId, post.postId, "containerOf")
    indexer.addEdge(indexer.newId(), post.postId, post.countryId, "isLocatedIn")
    for (tag <- post.tagIds)
      indexer.addEdge(indexer.newId(), post.postId, tag, "hasTag")
  }
  ...
  def update(comment: Update7AddComment): Unit = {
    ...
    indexer.addEdge(indexer.newId(), comment.commentId, comment.authorPersonId, "hasCreator")
    indexer.addEdge(indexer.newId(), comment.commentId,
      Math.max(comment.replyToCommentId, comment.replyToPostId), "replyOf")
    for (tag <- comment.tagIds)
      indexer.addEdge(indexer.newId(), comment.commentId, tag, "hasTag")
  }
