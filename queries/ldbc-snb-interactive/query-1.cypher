...
OPTIONAL MATCH (friend)-[studyAt:studyAt]->(uni:University)-[:isLocatedIn]->(uniCity:City)
WITH friend,
  collect([uni.name, studyAt.classYear, uniCity.name]) AS unis,
  friendCity,
  distance
OPTIONAL MATCH (friend)-[worksAt:worksAt]->(company:Company)-[:isLocatedIn]->(companyCountry:Country)
WITH friend,
