...
(defn- expand-implications [constr-defs]
  (apply union (map (fn [[constr vars]]
                      (constraint/implies* (constraint/->ConstraintBinding constr vars)))
                    constr-defs)))

(defn op
  ([name vars reqs sats] (let [req-set (expand-implications (partition 2 reqs))
                               sat-set (difference (expand-implications (partition 2 sats))
                                                   req-set)]
                           (map->Op {:name      name
                                     :vars      vars
                                     :requires  (constraint/constraint-bindings-to-map (into () req-set))
                                     :satisfies (constraint/constraint-bindings-to-map (into () sat-set))})))
...
(defmacro defop
  ...
  (let [
        ...
        [reqs sats] (reqs-sats rest)]
    `(let [type# (def ~name (op '~(symbol (str *ns*) (str name))
                                ~(walk/postwalk-replace var-map vars)
                                [~@(walk/postwalk-replace var-map reqs)]
                                [~@(walk/postwalk-replace var-map sats)]))]
