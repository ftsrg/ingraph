  ...
  def createQueryEngine(plan: ENode, indexer: Indexer): AnnotatedRelationalEngine =
    new AnnotatedRelationalEngine {
      ...
      private def sortAndTop(op: SortAndTop, expr: ForwardConnection) = {
        ...
        val sortKeys = op.inode.order.map(e => ExpressionParser.parseValue(e, variableLookup)).toVector
      ...
      private def create(op: Create, indexer: Indexer, expr: ForwardConnection) = {
        ...
        (m: ReteMessage) => {
          m match {
            case cs: IncrementalChangeSet => creations.foreach(r => cs.positive.foreach(r))
            case _ =>
          }
          expr.child(m)
        }
      ...
      private def delete(op: Delete, indexer: Indexer, expr: ForwardConnection) = {
        ...
        (m: ReteMessage) => {
          m match {
            case cs: IncrementalChangeSet => removals.foreach(r => cs.positive.foreach(r))
            case _ =>
          }
          expr.child(m)
        }
