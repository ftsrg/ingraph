...
class BiEngineTest extends EngineTest {
  ...
  test("bi-02: Top tags for country, age, gender, time") {
    val stages = run(
  ...
  test("bi-05: Top posters in a country") {
    val stages = run(
  ...
  test("bi-10: Central Person for a Tag") {
    val stages = run(
  ...
  test("bi-21: Zombies in a country") {
    val stages = run(
