select p_personid, p_firstname, p_lastname,
       ( select count(distinct m_messageid)
         ...
         exists (select * from person_tag where pt_personid = 30786325579101 and pt_tagid = pt1.mt_tagid)
       ) -
       ( select count(*)
         from message
         where
         m_creatorid = p_personid and
         m_c_replyof IS NULL and -- post, not comment
         not exists (select * from person_tag, message_tag where pt_personid = 30786325579101 and pt_tagid = mt_tagid and mt_messageid = m_messageid)
       ) as score,
       p_gender, pl_name
from person, place,
...
(
	(extract(month from to_timestamp(p_birthday)) = 7 and (case when extract(day from to_timestamp(p_birthday)) >= 21 then true else false end))
	or
	(extract(month from to_timestamp(p_birthday)) = 8 and (case when extract(day from to_timestamp(p_birthday)) <  22 then true else false end))
)
