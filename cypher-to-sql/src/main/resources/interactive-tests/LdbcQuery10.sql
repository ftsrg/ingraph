...
(
	(extract(month from p_birthday) = 7 and (case when extract(day from p_birthday) >= 21 then true else false end))
	or
	(extract(month from p_birthday) = 8 and (case when extract(day from p_birthday) <  22 then true else false end))
)
