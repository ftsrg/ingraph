  ...
  def getSql(node: Any, options: CompilerOptions): String = {
    val sqlString =
      node match {
        ...
        case ref: SubqueryAttributeReference => {
          val prefix = getQuotedColumnName(ref.subqueryName)
          val attributeName = getQuotedColumnName(ref.child)

          s"$prefix.$attributeName"
        }
