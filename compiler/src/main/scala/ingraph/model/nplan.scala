...
trait JoinLike extends BinaryNNode {
...
// leaf nodes
case class GetVertices(v: VertexAttribute) extends LeafNNode {
...
case class Dual() extends LeafNNode {
  override def output = Seq()
}

// unary nodes
case class AllDifferent(edges: Seq[AbstractEdgeAttribute],
                        child: NNode) extends UnaryNNode {}

case class DuplicateElimination(child: NNode) extends UnaryNNode {}

case class Production(child: NNode) extends UnaryNNode {}

abstract class AbstractProjection(projectList: TProjectList, child: NNode) extends UnaryNNode with ProjectionDescriptor {
...
case class Grouping(aggregationCriteria: Seq[Expression], projectList: TProjectList, child: NNode) extends UnaryNNode with ProjectionDescriptor {
  override def output = projectList
}
...
case class Unwind(unwindAttribute: UnwindAttribute, child: NNode) extends UnaryNNode {
...
case class Union(bag: Boolean,
                 left: NNode,
                 right: NNode) extends BinaryNNode {
  override def output: Seq[ResolvableName] = left.output
}

case class AntiJoin(left: NNode, right: NNode) extends BinaryNNode with JoinLike {
  override def output: Seq[ResolvableName] = left.output
}
...
case class ThetaLeftOuterJoin(left: NNode,
                              right: NNode,
                              condition: Expression) extends BinaryNNode with EquiJoinLike {
}

// DML operators
abstract class CudOperator(child: NNode) extends UnaryNNode {}

case class Create(attribute: ResolvableName, child: NNode) extends CudOperator(child) {
  override def output: Seq[ResolvableName] = child.output ++ Seq(attribute)
}

case class Delete(attributes: Seq[ResolvableName], detach: Boolean, child: NNode) extends CudOperator(child) {}

case class Merge(attributes: ResolvableName, child: NNode) extends CudOperator(child) {}

case class SetNode(vertexLabelUpdates: Set[VertexLabelUpdate], child: NNode) extends CudOperator(child) {}

case class Remove(vertexLabelUpdates: Set[VertexLabelUpdate], child: NNode) extends CudOperator(child) {}
