...
\paragraph{Running example.} \cref{fig:running-example-property-graph} presents an example inspired by the Movie Database dataset\footnote{\url{https://neo4j.com/developer/movie-database/}}.
...
The $\cartesianproductop$ operator produces the \cartesianproducttext $ t = r \cartesianproductop s.$ The result of the \jointext operator $\joinop$ is determined by creating the Cartesian product of the relations, then filtering those tuples which are equal on the attributes that share a common name. The combined tuples are projected: from the attributes present in both of the two input relations, we only keep the ones in $r$ and drop the ones in $s$. Thus, the join operator is defined as
...
\paragraph{Summary.} \cref{table:collections} provides an overview of the operators of \rga.
